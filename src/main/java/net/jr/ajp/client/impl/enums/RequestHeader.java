...
/**
 * common request headers that are recognized by the protocol. these header names are turned into numerical values when sending request
 * 
 * @author jrialland
 * 
 */
public enum RequestHeader {
	...
	//@formatter:off
		...
		AcceptCharset("accept-charset"	,0xA002	),
		AcceptEncoding("accept-encoding",	0xA003	),
		AcceptLanguage("accept-language",	0xA004	),
		Authorization("authorization",	0xA005	),
		Connection("connection"	,0xA006	),
		ContentType("content-type"	,0xA007	),
		ContentLength("content-length",	0xA008	),
		Cookie("cookie"	,0xA009	),
		Cookie2("cookie2",	0xA00A	),
		Host("host"	,0xA00B	),
		Pragma("pragma"	,0xA00C	),
		Referer("referer",	0xA00D	),
		UserAgent("user-agent",	0xA00E	);
		//@formatter:on
